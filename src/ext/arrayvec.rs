...
impl<const N: usize> Encoder<ArrayString<N>> for StrEncoder {
    ...
    fn encode(&mut self, t: &ArrayString<N>) {
        // Only lengths < 255 are fast to encode and avoid copying lots of
        // memory for 1 byte strings. TODO miri doesn't like
        // ArrayString::as_str().as_ptr(), replace with ArrayString::as_ptr()
        // when available.
        if N > 64 || cfg!(miri) {
        ...
        primitives.reserve(N); // TODO Buffer::reserve impl additional * N so we can remove
                               // encode_vectored impl.
        let dst = primitives.end_ptr();

        // Safety: `s.as_ptr()` points to `N` valid bytes since it's
        // referencing an ArrayString<N>. `dst` has enough space for
        // `[T; N]` because we've reserved `N`.
        unsafe {
            *(dst as *mut MaybeUninit<[u8; N]>) =
                *(s.as_ptr() as *const MaybeUninit<[u8; N]>);
    ...
    fn encode_vectored<'a>(
        &mut self,
        i: impl Iterator<Item = &'a ArrayString<N>> + Clone,
    ) {
        // Only lengths < 255 are fast to encode and avoid copying lots of
        // memory for 1 byte strings. TODO miri doesn't like
        // ArrayString::as_str().as_ptr(), replace with ArrayString::as_ptr()
        ...
        // This encode_vectored impl is same as encode impl, but pulls the
        // reserve out of the loop.
        let primitives = self.0.elements.as_primitive().unwrap();
        ...
        for t in i {
            ...
            self.0.lengths.encode_less_than_255(s.len());
            // Safety: `s.as_ptr()` points to `N` valid bytes since it's
            // referencing an ArrayString<N>. `dst` has enough
            // space for `[T; N]` because we've reserved `size_hint * N`.
            unsafe {
                *(dst as *mut MaybeUninit<[u8; N]>) =
                    *(s.as_ptr() as *const MaybeUninit<[u8; N]>);
                dst = dst.add(s.len());
...
// TODO replace with StrDecoder<N> that optimizes calls to
// LengthDecoder<N>::decode.
#[derive(Default)]
...
impl<'a, const N: usize> Decoder<'a, ArrayString<N>>
    for ArrayStringDecoder<'a, N>
{
    #[inline(always)]
    fn decode_in_place(&mut self, out: &mut MaybeUninit<ArrayString<N>>) {
        ...
        // TODO miri doesn't like ArrayString::as_mut_str().as_mut_ptr(),
        // replace with ArrayString::as_mut_ptr() when available.
        if N > 64 || cfg!(miri) {
        ...
        // Safety: We just checked n != 0 and ensured `self.lengths.max_len()
        // <= N` in populate. Also, `dst` has room for `[u8; N]` since
        // it's an ArrayString<N>.
        unsafe {
            let src = s.as_ptr();
...
impl<T: Encode, const N: usize> Encoder<ArrayVec<T, N>> for VecEncoder<T> {
    ...
    fn encode_vectored<'a>(
        &mut self,
        i: impl Iterator<Item = &'a ArrayVec<T, N>> + Clone,
    ) where
        ArrayVec<T, N>: 'a,
    {
    ...
    for ArrayVecDecoder<'a, T, N>
{
    ...
    fn decode_in_place(&mut self, out: &mut MaybeUninit<ArrayVec<T, N>>) {
        ...
        unsafe {
            ...
            for i in 0..n {
                self.0.elements.decode_in_place(
                    &mut *(av.as_mut_ptr().add(i) as *mut MaybeUninit<T>),
                );
            }
            av.set_len(n);
        }
...
mod tests {
    ...
    // Smaller set of tests for ArrayString than ArrayVec they share
    // VecEncoder/LengthDecoder.
    #[test]
    fn array_string() {
        ...
        assert!(decode::<ArrayString<0>>(&encode(
            &ArrayString::<0>::default()
        ))
        .is_ok());
    }
    ...
    fn array_vec() {
        ...
        assert!(decode::<ArrayVec<u8, 0>>(&encode(
            &ArrayVec::<u8, 0>::default()
        ))
        .is_ok());

        // Make sure LengthDecoder::any_greater_than works on large lengths
        // too.
    ...
    fn array_string_bug() {
        ...
        // Put the buffer in a state where its ArrayStringDecoder's
        // LengthDecoder has a len of 1. If this didn't error, the
        // decoder would end up with a len of 0 since all items would be
        // consumed.
        buffer.decode::<Vec<T>>(&encode::<Vec<T>>(&vec![v])[..2]).unwrap_err();
        // Now decode 0 ArrayStrings. Previously LengthDecoder wouldn't get
        // populated if StrDecoder was passed `length` of 0. This
        // results in a debug assert failing in
        // LengthDecoder::any_greater_than, since FastSlice::as_slice is called
        // with len of 0, but the FastSlice has a len of 1 from before.
